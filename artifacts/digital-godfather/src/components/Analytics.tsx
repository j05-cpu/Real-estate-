...
function MetricCard({ icon, iconBg, iconGlow, title, value, change, bar, barColor, sub }: {
  ...
  return (
    <div
      className="bg-slate-800/60 rounded-2xl p-5 backdrop-blur-sm"
      style={{ border: `1px solid ${iconGlow}44`, boxShadow: `0 0 20px ${iconGlow}26, inset 0 0 20px ${iconGlow}0d` }}
    >
...
export default function Analytics() {
  const { leads, loading, error, usingFallback, reload } = useLeads();
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            Monday, 23 June 2026 · Panvel Projects
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={reload}
            disabled={loading}
            className="hidden md:flex items-center gap-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 text-sm font-medium px-3 py-2 rounded-xl transition-colors disabled:opacity-50"
          >
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <span className="text-xs text-slate-500">Lead breakdown:</span>
        ...
        <span className="text-xs text-blue-400 bg-blue-900/20 border border-blue-800/40 px-2.5 py-1 rounded-full font-semibold">{leads.length - hotCount - warmCount} Cold</span>
      </div>
      ...
      <div className="bg-slate-800/60 rounded-2xl border border-slate-700/50 overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-700/50 flex items-center justify-between flex-wrap gap-3">
          <div>
            <h2 className="text-base font-semibold text-white">Live Lead Tracker</h2>
            <p className="text-xs text-slate-500 mt-0.5">Rohan AI-qualified leads · Real-time</p>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-2 bg-slate-900 border border-slate-700 rounded-xl px-3 py-2">
              ...
              <input
                ...
                className="bg-transparent text-sm text-slate-300 placeholder-slate-600 outline-none w-28 md:w-40"
              />
            </div>
          </div>
        </div>
